 ...
 */
export function getClusteringFields(platform) {
 ...
 */
export function getBigQueryTableConfig(platform) {
    const config = {
        timePartitioning: {
            type: 'DAY',
            field: 'date',
 ...
 * - date is NOT NULL
 */
export function validateSchemaForPartitioning(schema, platform) {
    const errors = [];
 ...
 */
export function getTableCreationOptions(platform, schema) {
 ...
 */
export function generateCreateTableSQL(tableName, platform, schema) {
    const config = getBigQueryTableConfig(platform);
 ...
 */
export function needsPartitionMigration(tableMetadata) {
    const missing = [];
...
//# sourceMappingURL=bigquery-table-config.js.map