 ...
 * ```
 */
export function injectGuidance(data, guidanceText) {
    return {
        content: [{
                type: 'text',
                text: guidanceText
 ...
 * ```
 */
export function formatNextSteps(suggestions) {
 ...
 * ```
 */
export function formatDryRunPreview(title, step, changes, risks = [], recommendations = []) {
 ...
 * ```
 */
export function checkRequiredParams(input, required) {
    const missing = required.filter(param => input[param] === undefined || input[param] === null || input[param] === '');
    return {
 ...
 * ```
 */
export function formatDataTable(data, columns, maxRows = 10) {
    if (data.length === 0)
        return '(No data)';
 ...
 * ```
 */
export function formatPercentageChange(oldValue, newValue) {
...
//# sourceMappingURL=interactive-workflow.js.map