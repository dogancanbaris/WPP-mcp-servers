...
What date range would you like to analyze?`;
            ...
            const insights = [];
            if (avgCTR > 0.05)
                insights.push('✅ Good overall CTR (>5%)');
            else if (avgCTR > 0.02)
                insights.push('⚠️ CTR could be improved (2-5%)');
            else
                insights.push('🔴 Low CTR (<2%) - optimize titles and descriptions');
            if (avgPosition < 10)
                insights.push('✅ Strong average ranking (top 10)');
            else if (avgPosition < 20)
                insights.push('⚠️ Rankings need improvement (11-20)');
            else
                insights.push('🔴 Poor rankings (>20) - focus on SEO');
