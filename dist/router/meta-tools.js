...
import { searchToolsByKeyword, getToolsByCategory, getToolsByPlatform, getAllCategories, findCategoryForTool, } from './tool-categories.js';
...
${categories.map(({ key, category }) => `• ${key} - ${category.description} (${category.tools.length} tools)`).join('\n')}
...
${toolsWithMetadata.map((tool, i) => `${i + 1}. **${tool.name}**
   ${tool.description}
   Category: ${tool.category || 'N/A'} | Platform: ${tool.platform || 'N/A'}`).join('\n\n')}
...
Or browse categories:
  search_tools({})`
            ...
            const paramsDoc = Object.entries(schemaProps).map(([key, prop]) => {
 ...
 */
export function getMetaTools(registry) {
    return [
