...
3. Check back in 24-48 hours after keywords start serving`;
            ...
            const analysis = adGroups.map((ag) => {
                const avgQualityScore = ag.qualityScores.scores.length > 0
                    ? ag.qualityScores.scores.reduce((sum, s) => sum + s, 0) / ag.qualityScores.scores.length
                    : null;
                const avgCreativeScore = ag.qualityScores.creativeScores.length > 0
                    ? ag.qualityScores.creativeScores.reduce((sum, s) => sum + s, 0) / ag.qualityScores.creativeScores.length
                    : null;
                const avgPostClickScore = ag.qualityScores.postClickScores.length > 0
                    ? ag.qualityScores.postClickScores.reduce((sum, s) => sum + s, 0) / ag.qualityScores.postClickScores.length
                    : null;
                // Find low-scoring keywords (QS < 5)
                const lowQualityKeywords = ag.keywords.filter((k) => k.qualityScore && k.qualityScore < 5);
                // Find high-scoring keywords (QS >= 8)
                const highQualityKeywords = ag.keywords.filter((k) => k.qualityScore && k.qualityScore >= 8);
                return {
                    adGroup: ag.adGroup,
            ...
            // Overall statistics
            const totalKeywords = analysis.reduce((sum, a) => sum + a.keywordCount, 0);
            const totalLowQuality = analysis.reduce((sum, a) => sum + a.lowQualityCount, 0);
            const totalHighQuality = analysis.reduce((sum, a) => sum + a.highQualityCount, 0);
            const overallAvgQS = analysis.reduce((sum, a) => sum + (a.avgQualityScore || 0), 0) / analysis.length;
            // Build rich guidance
...
${analysis.slice(0, 5).map((a, i) => {
                ...
                const avgCpc = a.metrics.clicks > 0 ? (a.metrics.costMicros / 1000000) / a.metrics.clicks : 0;
...
${analysis
                .flatMap((a) => a.lowQualityKeywords.map((k) => ({ ...k, adGroupName: a.adGroup?.name })))
...
Full Quality Score data (${totalKeywords} keywords) available in structured output.`;
            ...
            logger.error('Failed to get Quality Score data', error);
            throw error;
        }
    },
};
//# sourceMappingURL=get-ad-group-quality-score.tool.js.map