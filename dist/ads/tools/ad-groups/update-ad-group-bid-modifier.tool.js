...
Create an ad group first using create_ad_group.`;
                ...
                return formatDiscoveryResponse({
                    ...
                    itemFormatter: (ag, i) => {
                        ...
                        const avgCpc = ag.metrics.clicks > 0
                            ? formatCurrency(ag.metrics.costMicros / 1000000 / ag.metrics.clicks)
                            : '$0.00';
   ...
   Avg CPC: ${avgCpc} | Conversions: ${ag.metrics.conversions.toFixed(2)}`;
        ...
        `);
                ...
                // Aggregate metrics
                const totalImpressions = adGroupDetails.reduce((sum, row) => sum + (row.metrics?.impressions || 0), 0);
                const totalClicks = adGroupDetails.reduce((sum, row) => sum + (row.metrics?.clicks || 0), 0);
                const totalCostMicros = adGroupDetails.reduce((sum, row) => sum + (row.metrics?.cost_micros || 0), 0);
                const totalConversions = adGroupDetails.reduce((sum, row) => sum + (row.metrics?.conversions || 0), 0);
                const avgCpc = totalClicks > 0 ? (totalCostMicros / 1000000) / totalClicks : 0;
...
**Recent Performance (Last 30 Days):**
   ...
   • Total Cost: ${formatCurrency(totalCostMicros / 1000000)}
...
\`\`\``;
      ...
      `);
            ...
            const currentBid = currentBidMicros / 1000000;
            const newBid = currentBid * (1 + percentageChange / 100);
            const newBidMicros = Math.max(10000, Math.round(newBid * 1000000)); // Min $0.01
            ...
            const successText = formatSuccessSummary({
                ...
                details: {
                    ...
                    'Percentage Change': `${percentageChange > 0 ? '+' : ''}${percentageChange}%`,
                    'New Bid': formatCurrency(newBidMicros / 1000000),
                    'Dollar Change': formatCurrency((newBidMicros / 1000000) - currentBid),
                },
                ...
                warnings: Math.abs(percentageChange) > 30
            ...
            logger.error('Failed to update ad group bid', error);
            // AUDIT: Log failed operation
            await audit.logFailedOperation('user', 'update_ad_group_bid_modifier', input.customerId, error.message, {
...
//# sourceMappingURL=update-ad-group-bid-modifier.tool.js.map