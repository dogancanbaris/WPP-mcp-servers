...
3. Enable the ad group when ready`;
            ...
            // Analyze ad groups
            const statusCounts = adGroups.reduce((acc, ag) => {
                const status = ag.ad_group?.status || 'UNKNOWN';
                acc[status] = (acc[status] || 0) + 1;
                return acc;
            }, {});
            // Calculate totals
            const totals = adGroups.reduce((acc, ag) => {
            ...
            // Find top performers
            const byImpressions = [...adGroups].sort((a, b) => (b.metrics?.impressions || 0) - (a.metrics?.impressions || 0));
            const byConversions = [...adGroups].sort((a, b) => (b.metrics?.conversions || 0) - (a.metrics?.conversions || 0));
            // Quality Score analysis (if available)
            const withQualityScore = adGroups.filter((ag) => ag.ad_group?.quality_info?.quality_score);
            const avgQualityScore = withQualityScore.length > 0
                ? withQualityScore.reduce((sum, ag) => sum + (ag.ad_group.quality_info.quality_score || 0), 0) / withQualityScore.length
                : null;
            // Build rich guidance
...
${byImpressions.slice(0, 3).map((ag, i) => {
...
**Top Converters:**
${byConversions.slice(0, 3).map((ag, i) => {
                const adGroup = ag.ad_group;
                const metrics = ag.metrics || {};
...
Which ad group would you like to analyze or modify?`;
            ...
            logger.error('Failed to list ad groups', error);
            throw error;
        }
    },
};
//# sourceMappingURL=list-ad-groups.tool.js.map