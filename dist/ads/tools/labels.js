...
import { formatDiscoveryResponse, injectGuidance, formatNextSteps, formatSuccessSummary } from '../../shared/interactive-workflow.js';
...
${formatNextSteps([
            ...
            logger.error('[list_labels] Error:', error);
            throw error;
        }
    },
};
...
What should the label be named?`;
            ...
            logger.error('[create_label] Error:', error);
            // AUDIT: Log failed label creation
            await audit.logFailedOperation('user', 'create_label', input.customerId, error.message, {
    ...
    async handler(input) {
        try {
            ...
            if (!labelId) {
                const labels = await client.listLabels(customerId);
                if (labels.length === 0) {
                    return injectGuidance({ customerId }, `🗑️ NO LABELS FOUND (Step 2/2)
...
Create a label first using create_label.`);
   ...
   ⚠️ WARNING: Removing this label will remove it from ALL resources`;
            ...
            const labels = await client.listLabels(customerId);
            const labelToRemove = labels.find((l) => l.label.id === labelId);
            ...
            logger.error('[remove_label] Error:', error);
            // AUDIT: Log failed label removal
            await audit.logFailedOperation('user', 'remove_label', input.customerId, error.message, {
...
export const applyCampaignLabelTool = {
   ...
   Type: ${campaign.advertising_channel_type}`;
            ...
            if (!labelId) {
                const labels = await client.listLabels(customerId);
                if (labels.length === 0) {
                    return injectGuidance({ customerId, campaignId }, `🏷️ NO LABELS FOUND (Step 3/3)
...
Create a label first using create_label, then apply it to campaigns.`);
   ...
   Status: ${label.status}`;
            ...
            const labels = await client.listLabels(customerId);
            const label = labels.find((l) => l.label.id === labelId);
            ...
            logger.error('[apply_campaign_label] Error:', error);
            // AUDIT: Log failed label application
            await audit.logFailedOperation('user', 'apply_campaign_label', input.customerId, error.message, {
...
export const applyAdGroupLabelTool = {
   ...
   Campaign: ${campaign.name} (${campaign.id})`;
            ...
            if (!labelId) {
                const labels = await client.listLabels(customerId);
                if (labels.length === 0) {
                    return injectGuidance({ customerId, adGroupId }, `🏷️ NO LABELS FOUND (Step 3/3)
...
Create a label first using create_label, then apply it to ad groups.`);
   ...
   Status: ${label.status}`;
      ...
      `);
            ...
            logger.error('[apply_ad_group_label] Error:', error);
            // AUDIT: Log failed label application
            await audit.logFailedOperation('user', 'apply_ad_group_label', input.customerId, error.message, {
        ...
        try {
            ...
            if (!criterionId) {
                ...
                if (keywords.length === 0) {
                    return injectGuidance({ customerId }, `🏷️ NO KEYWORDS FOUND (Step 2/3)
...
Add keywords first using add_keywords, then apply labels to them.`);
   ...
   Criterion ID: ${criterion.criterion_id}`;
            ...
            if (!labelId) {
                const labels = await client.listLabels(customerId);
                if (labels.length === 0) {
                    return injectGuidance({ customerId, criterionId }, `🏷️ NO LABELS FOUND (Step 3/3)
...
Create a label first using create_label, then apply it to keywords.`);
   ...
   Status: ${label.status}`;
            ...
            // Get names for audit trail
            const keywords = await client.listKeywords(customerId);
            const keyword = keywords.find((kw) => kw.ad_group_criterion.criterion_id === criterionId);
            ...
            logger.error('[apply_keyword_label] Error:', error);
            // AUDIT: Log failed label application
            await audit.logFailedOperation('user', 'apply_keyword_label', input.customerId, error.message, {
