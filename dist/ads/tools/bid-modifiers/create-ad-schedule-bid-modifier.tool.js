...
What percentage adjustment for ${timeBlock}?`;
                return injectGuidance({ customerId, campaignId, dayOfWeek, startHour, endHour }, guidanceText);
            }
            // Validate modifier range
            if (bidModifierPercent < -90 || bidModifierPercent > 900) {
                throw new Error('Bid modifier must be between -90% and +900%. Provided: ' + bidModifierPercent);
            }
            // ═══ STEP 7: DRY-RUN PREVIEW ═══
            const approvalEnforcer = getApprovalEnforcer();
            const dryRunBuilder = new DryRunResultBuilder('create_ad_schedule_bid_modifier', 'Google Ads', customerId);
            const formatTime = (hour) => {
                if (hour === 0)
                    return '12am';
                if (hour === 24)
            ...
            if (Math.abs(bidModifierPercent) >= 50) {
                dryRunBuilder.addRecommendation(`⚠️ Large bid adjustment (${bidModifierPercent}%) for ${timeBlock}. This will significantly impact spend during this time period.`);
            }
            if (Math.abs(bidModifierPercent) < 10) {
                dryRunBuilder.addRecommendation(`ℹ️ Small adjustment (${bidModifierPercent}%). May have minimal impact during ${timeBlock}.`);
            }
            const hourDuration = endHour - startHour;
            if (hourDuration >= 12) {
                dryRunBuilder.addRecommendation(`📊 Large time block (${hourDuration} hours). Consider breaking into smaller blocks for more granular optimization.`);
            }
            dryRunBuilder.addRecommendation(`⏰ Monitor ${timeBlock} performance after this change. Check "Ad schedule" report in 14-30 days.`);
            dryRunBuilder.addRecommendation(`🌍 Timezone: This schedule uses your account timezone. Verify it matches your target audience timezone.`);
            const dryRun = dryRunBuilder.build();
            // If no confirmation token, return preview
            if (!confirmationToken) {
                const { confirmationToken: token } = await approvalEnforcer.createDryRun('create_ad_schedule_bid_modifier', 'Google Ads', customerId, { campaignId, dayOfWeek, startHour, endHour, bidModifierPercent });
                ...
                return {
                    ...
                    message: 'Ad schedule bid modifier creation requires approval. Review the preview and call this tool again with confirmationToken.',
                };
            }
            ...
            logger.error('Failed to create ad schedule bid modifier', error);
            await audit.logFailedOperation('user', 'create_ad_schedule_bid_modifier', input.customerId, error.message, {
...
//# sourceMappingURL=create-ad-schedule-bid-modifier.tool.js.map