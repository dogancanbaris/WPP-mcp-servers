...
What percentage adjustment?`;
            ...
            const dryRunBuilder = new DryRunResultBuilder('create_device_bid_modifier', 'Google Ads', customerId);
            ...
            if (Math.abs(bidModifierPercent) >= 50) {
                dryRunBuilder.addRecommendation(`⚠️ Large bid adjustment (${bidModifierPercent}%). This will significantly impact spend. Monitor closely.`);
            }
            if (bidModifierPercent === -100) {
                dryRunBuilder.addRecommendation(`🚫 EXCLUSION: Ads will NOT show on ${deviceType}. Ensure this is intentional.`);
            }
            if (Math.abs(bidModifierPercent) < 10) {
                dryRunBuilder.addRecommendation(`ℹ️ Small adjustment (${bidModifierPercent}%). May have minimal impact. Consider larger adjustment if performance gap is significant.`);
            }
            dryRunBuilder.addRecommendation(`📊 Monitor ${deviceType} performance after this change. Check "Devices" report in 7-14 days.`);
            const dryRun = dryRunBuilder.build();
            // If no confirmation token, return preview
            if (!confirmationToken) {
                const { confirmationToken: token } = await approvalEnforcer.createDryRun('create_device_bid_modifier', 'Google Ads', customerId, { campaignId, deviceType, bidModifierPercent });
                ...
                return {
                    ...
                    message: 'Device bid modifier creation requires approval. Review the preview and call this tool again with confirmationToken.',
                };
            }
            ...
            logger.error('Failed to create device bid modifier', error);
            await audit.logFailedOperation('user', 'create_device_bid_modifier', input.customerId, error.message, {
...
//# sourceMappingURL=create-device-bid-modifier.tool.js.map