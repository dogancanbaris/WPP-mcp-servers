...
A -20% modifier for 18-24 on $1.00 bids = $0.80 effective bid`,
    inputSchema: {
        ...
        properties: {
            ...
            demographicValue: {
                type: 'string',
                description: 'Demographic value (e.g., AGE_25_34, AGE_35_44, MALE, FEMALE)',
            },
            bidModifierPercent: {
                type: 'number',
    ...
    async handler(input) {
        try {
            const { customerId, adGroupId, demographicType, demographicValue, bidModifierPercent, confirmationToken } = input;
        ...
        `);
                if (adGroups.length === 0) {
                    return injectGuidance({ customerId }, '⚠️ NO AD GROUPS FOUND\n\nCreate an ad group first using create_ad_group tool.');
                }
                return formatDiscoveryResponse({
                    step: '2/5',
...
Which gender?`;
            ...
            if (bidModifierPercent === undefined) {
                const demographicLabel = demographicType === 'AGE'
...
What percentage adjustment for ${demographicLabel}?`;
                return injectGuidance({ customerId, adGroupId, demographicType, demographicValue }, guidanceText);
            }
            ...
            const dryRunBuilder = new DryRunResultBuilder('create_demographic_bid_modifier', 'Google Ads', customerId);
            const demographicLabel = demographicType === 'AGE'
                ? demographicValue.replace('AGE_', '').replace('_', '-')
                : demographicValue;
            ...
            if (Math.abs(bidModifierPercent) >= 50) {
                dryRunBuilder.addRecommendation(`⚠️ Large bid adjustment (${bidModifierPercent}%) for ${demographicLabel}. This will significantly impact spend for this demographic.`);
            }
            if (bidModifierPercent === -100) {
                dryRunBuilder.addRecommendation(`🚫 EXCLUSION: Ads will NOT show to ${demographicLabel}. Ensure this complies with anti-discrimination policies and has business justification.`);
            }
            if (Math.abs(bidModifierPercent) < 10) {
                dryRunBuilder.addRecommendation(`ℹ️ Small adjustment (${bidModifierPercent}%). May have minimal impact on ${demographicLabel} performance.`);
            }
            dryRunBuilder.addRecommendation(`📊 Monitor ${demographicLabel} performance after this change. Check "Demographics" report in 14-30 days.`);
            dryRunBuilder.addRecommendation(`⚖️ Legal: Ensure demographic targeting complies with applicable laws and Google Ads policies.`);
            const dryRun = dryRunBuilder.build();
            // If no confirmation token, return preview
            if (!confirmationToken) {
                const { confirmationToken: token } = await approvalEnforcer.createDryRun('create_demographic_bid_modifier', 'Google Ads', customerId, { adGroupId, demographicType, demographicValue, bidModifierPercent });
                ...
                return {
                    ...
                    message: 'Demographic bid modifier creation requires approval. Review the preview and call this tool again with confirmationToken.',
                };
            }
            ...
            logger.error('Failed to create demographic bid modifier', error);
            await audit.logFailedOperation('user', 'create_demographic_bid_modifier', input.customerId, error.message, {
...
//# sourceMappingURL=create-demographic-bid-modifier.tool.js.map