...
**Provide:** status (ENABLED, PAUSED, or REMOVED)`;
        ...
        `);
                const currentStatus = adDetails[0]?.ad_group_ad?.status || 'UNKNOWN';
                const approvalStatus = adDetails[0]?.ad_group_ad?.policy_summary?.approval_status || 'UNKNOWN';
                const headlines = adDetails[0]?.ad_group_ad?.ad?.responsive_search_ad?.headlines?.map((h) => h.text).slice(0, 2) || [];
                const warnings = [];
...
\`\`\``;
      ...
      `);
            ...
            await audit.logFailedOperation('user', 'update_ad', input.customerId, error.message, {
...
//# sourceMappingURL=update-ad.tool.js.map