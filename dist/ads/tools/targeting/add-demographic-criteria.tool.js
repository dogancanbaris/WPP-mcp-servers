...
export const addDemographicCriteriaTool = {
    ...
    async handler(input) {
        try {
            const { customerId, campaignId, ageRanges, genders, parentalStatuses, householdIncomes, confirmationToken } = input;
...
**AGE RANGES:**
${AGE_RANGES.map((a) => `• ${a.name} - ${a.description} (ID: ${a.id})`).join('\n')}

**GENDERS:**
${GENDERS.map((g) => `• ${g.name} (ID: ${g.id})`).join('\n')}

**PARENTAL STATUS:**
${PARENTAL_STATUS.map((p) => `• ${p.name} - ${p.description} (ID: ${p.id})`).join('\n')}

**HOUSEHOLD INCOME (US only):**
${HOUSEHOLD_INCOME.map((h) => `• ${h.name} - ${h.description} (ID: ${h.id})`).join('\n')}
...
**Provide:** At least one demographic criterion (ageRanges, genders, parentalStatuses, or householdIncomes)`;
            ...
            if (!confirmationToken) {
                ...
                if (ageRanges && ageRanges.length > 0) {
                    const matched = ageRanges.map((id) => AGE_RANGES.find((a) => a.id === id)?.name || id);
                    dryRunBuilder.addChange(`Age Ranges: ${matched.join(', ')}`);
                    totalCriteria += ageRanges.length;
                }
                if (genders && genders.length > 0) {
                    const matched = genders.map((id) => GENDERS.find((g) => g.id === id)?.name || id);
                    dryRunBuilder.addChange(`Genders: ${matched.join(', ')}`);
                    totalCriteria += genders.length;
                }
                if (parentalStatuses && parentalStatuses.length > 0) {
                    const matched = parentalStatuses.map((id) => PARENTAL_STATUS.find((p) => p.id === id)?.name || id);
                ...
                if (householdIncomes && householdIncomes.length > 0) {
                    const matched = householdIncomes.map((id) => HOUSEHOLD_INCOME.find((h) => h.id === id)?.name || id);
            ...
            logger.error('Failed to add demographic criteria', error);
            await audit.logFailedOperation('user', 'add_demographic_criteria', input.customerId, error.message, {
...
//# sourceMappingURL=add-demographic-criteria.tool.js.map