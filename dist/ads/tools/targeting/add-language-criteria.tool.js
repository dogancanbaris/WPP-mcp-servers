...
**Common Languages:**
${COMMON_LANGUAGES.map((l) => `• ${l.name} (${l.nativeName}): languageIds: ["${l.id}"]`).join('\n')}
...
**Provide:** languageIds (array of language constant IDs)`;
            ...
            if (!confirmationToken) {
                ...
                const matchedLanguages = languageIds
                    .map((id) => COMMON_LANGUAGES.find((l) => l.id === id))
                    .filter(Boolean)
                    .map((l) => l.name);
            ...
            const operations = languageIds.map((languageId) => ({
            ...
            logger.error('Failed to add language criteria', error);
            await audit.logFailedOperation('user', 'add_language_criteria', input.customerId, error.message, { campaignId: input.campaignId, languageIds: input.languageIds });
            throw error;
        }
    },
};
//# sourceMappingURL=add-language-criteria.tool.js.map