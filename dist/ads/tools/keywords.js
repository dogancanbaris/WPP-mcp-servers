...
**Keywords Added:**
${keywords.map((kw, i) => `${i + 1}. "${kw.text}" [${kw.matchType}]${kw.maxCpcDollars ? ` @ $${kw.maxCpcDollars}/click` : ''}${kw.finalUrls ? ` → ${kw.finalUrls[0]}` : ''}`).join('\n')}
...
${adGroupId ? '- Use add_keywords to add keywords to this ad group' : '- Use list_campaigns to see all campaigns and ad groups'}`;
            ...
            const matchTypeCounts = results.reduce((acc, row) => {
            ...
            const qualityScores = results
                .map((row) => row.ad_group_criterion?.quality_info?.quality_score)
                .filter((qs) => qs !== null && qs !== undefined);
            const avgQualityScore = qualityScores.length > 0
                ? (qualityScores.reduce((sum, qs) => sum + qs, 0) / qualityScores.length).toFixed(1)
                : 'N/A';
            const lowQualityKeywords = results.filter((row) => row.ad_group_criterion?.quality_info?.quality_score && row.ad_group_criterion.quality_info.quality_score < 5).length;
            // Find top performers and underperformers
            const keywordsWithMetrics = results.filter((row) => (row.metrics?.clicks || 0) > 0);
...
${topPerformers.map((row, i) => {
                ...
                const cost = metrics.cost_micros ? (metrics.cost_micros / 1000000).toFixed(2) : '0.00';
...
- Use list_keywords to see all keywords in account`;
                ...
                const keywordList = results.map((row, i) => {
                    const kw = row.ad_group_criterion?.keyword;
                    const metrics = row.metrics || {};
...
- criterionIds (array of criterion IDs to remove)`;
      ...
      `;
            ...
            if (criterionIds.length > 50) {
                throw new Error(`Cannot remove ${criterionIds.length} keywords in one operation. Maximum is 50. Please batch into smaller operations.`);
            }
            // Build dry-run preview
            const approvalEnforcer = getApprovalEnforcer();
            const dryRunBuilder = new DryRunResultBuilder('remove_keywords', 'Google Ads', customerId);
            ...
            // Add warnings
            dryRunBuilder.addRisk(`Removing ${keywordDetails.length} keyword(s) is DESTRUCTIVE and cannot be easily undone`);
            const totalCost = keywordDetails.reduce((sum, row) => sum + (row.metrics?.cost_micros || 0), 0) / 1000000;
            if (totalCost > 100) {
                dryRunBuilder.addRisk(`These keywords have spent $${totalCost.toFixed(2)} historically - verify you want to remove them`);
            }
            const hasConversions = keywordDetails.some((row) => (row.metrics?.conversions || 0) > 0);
            if (hasConversions) {
                dryRunBuilder.addRisk(`Some keywords have generated conversions - removing may impact performance`);
            }
            dryRunBuilder.addRecommendation('Consider pausing keywords instead of removing if you want to test disabling them first');
            const dryRun = dryRunBuilder.build();
            // If no confirmation token, return preview
            if (!confirmationToken) {
                const { confirmationToken: token } = await approvalEnforcer.createDryRun('remove_keywords', 'Google Ads', customerId, { adGroupId, criterionIds });
                ...
                return {
                    ...
                    message: 'Keyword removal requires approval. Review the preview above and call this tool again with the confirmationToken to proceed.',
                };
            }
            ...
            const result = await approvalEnforcer.validateAndExecute(confirmationToken, dryRun, async () => {
                ...
                const operations = criterionIds.map((criterionId) => ({
                    resource_name: `customers/${customerId}/adGroupCriteria/${adGroupId}~${criterionId}`,
                }));
                return await customer.adGroupCriteria.remove(operations);
            });
            return {
                success: true,
                data: {
            ...
            logger.error('Failed to remove keywords', error);
            throw error;
        }
    },
};
...
- Use add_keywords to add keywords first`;
                ...
                const keywordList = results.map((row, i) => {
                    ...
                    const currentBid = row.ad_group_criterion?.cpc_bid_micros
                        ? (row.ad_group_criterion.cpc_bid_micros / 1000000).toFixed(2)
                        : 'Not set';
                    const avgCpc = metrics.average_cpc ? (metrics.average_cpc / 1000000).toFixed(2) : '0.00';
                    const qs = row.ad_group_criterion?.quality_info?.quality_score;
                    return `${i + 1}. "${kw?.text}" [${kw?.match_type}]
...
- newBidDollars (e.g., 2.50 for $2.50)`;
                return injectGuidance({ customerId, adGroupId, criterionId }, guidanceText);
            }
            // ═══ STEP 4: DRY-RUN PREVIEW ═══
      ...
      `;
            ...
            const currentBid = currentBidMicros / 1000000;
            ...
            // Build dry-run
            const approvalEnforcer = getApprovalEnforcer();
            const dryRunBuilder = new DryRunResultBuilder('set_keyword_bid', 'Google Ads', customerId);
            dryRunBuilder.addChange({
            ...
            if (Math.abs(bidChange) > currentBid * 0.5) {
                dryRunBuilder.addRisk(`Large bid change (${bidChangePercent}%) may cause significant traffic/spend changes`);
                dryRunBuilder.addRecommendation('Consider making smaller incremental changes (10-20%) and monitoring results');
            }
            if (newBidDollars > 10) {
                dryRunBuilder.addRisk(`High CPC bid ($${newBidDollars}) may increase spend rapidly`);
            }
            if (bidChange > 0) {
                dryRunBuilder.addRecommendation('Monitor impression share and position after bid increase');
            }
            else {
                dryRunBuilder.addRecommendation('Monitor impression share - may decrease after bid reduction');
            }
            const dryRun = dryRunBuilder.build();
            // If no confirmation, return preview
            if (!confirmationToken) {
                const { confirmationToken: token } = await approvalEnforcer.createDryRun('set_keyword_bid', 'Google Ads', customerId, { adGroupId, criterionId, newBidDollars });
                const preview = approvalEnforcer.formatDryRunForDisplay(dryRun);
                return {
                    success: true,
                    requiresApproval: true,
            ...
            logger.error('Failed to set keyword bid', error);
            throw error;
        }
    },
};
...
- Use add_negative_keywords if you want to add negatives`;
                ...
                const negativeList = results.map((row, i) => {
...
\`\`\``;
      ...
      `;
            ...
            // Build dry-run
            const approvalEnforcer = getApprovalEnforcer();
            const dryRunBuilder = new DryRunResultBuilder('remove_negative_keywords', 'Google Ads', customerId);
            // Add each negative as change
            negativeDetails.forEach((row) => {
                const kw = row.campaign_criterion?.keyword;
                const criterionId = row.campaign_criterion?.criterion_id;
                dryRunBuilder.addChange({
            ...
            dryRunBuilder.addRisk(`Removing ${negativeDetails.length} negative keyword(s) will allow ads to show for previously blocked searches`);
            dryRunBuilder.addRisk('This may increase impressions, clicks, and spend on potentially irrelevant searches');
            dryRunBuilder.addRecommendation('Monitor search terms report after removal to ensure no wasted spend on irrelevant queries');
            const dryRun = dryRunBuilder.build();
            // If no confirmation, return preview
            if (!confirmationToken) {
                const { confirmationToken: token } = await approvalEnforcer.createDryRun('remove_negative_keywords', 'Google Ads', customerId, { campaignId, criterionIds });
                const preview = approvalEnforcer.formatDryRunForDisplay(dryRun);
                return {
                    success: true,
                    requiresApproval: true,
            ...
            const result = await approvalEnforcer.validateAndExecute(confirmationToken, dryRun, async () => {
                ...
                return await customer.campaignCriteria.remove(operations);
            });
            return {
                success: true,
                data: {
            ...
            logger.error('Failed to remove negative keywords', error);
            throw error;
        }
    },
};
...
- newMatchType (EXACT, PHRASE, or BROAD)`;
      ...
      `;
            ...
            // Build dry-run
            const approvalEnforcer = getApprovalEnforcer();
            const dryRunBuilder = new DryRunResultBuilder('update_keyword_match_type', 'Google Ads', customerId);
            dryRunBuilder.addChange({
            ...
            if (currentMatchType === 'EXACT' && newMatchType === 'BROAD') {
                dryRunBuilder.addRisk('Changing from EXACT to BROAD will significantly expand reach - may increase spend dramatically');
                dryRunBuilder.addRecommendation('Consider PHRASE match as intermediate step before going BROAD');
            }
            if (newMatchType === 'BROAD') {
                dryRunBuilder.addRisk('BROAD match requires close monitoring and strong negative keyword list');
                dryRunBuilder.addRecommendation('Review search terms report frequently after enabling BROAD match');
            }
            if (currentMatchType === 'BROAD' && newMatchType !== 'BROAD') {
                dryRunBuilder.addRecommendation(`Narrowing from BROAD to ${newMatchType} will reduce impressions but improve relevance`);
            }
            dryRunBuilder.addRecommendation('Google Ads API limitation: This will remove old keyword and add new one with updated match type');
            const dryRun = dryRunBuilder.build();
            // If no confirmation, return preview
            if (!confirmationToken) {
                const { confirmationToken: token } = await approvalEnforcer.createDryRun('update_keyword_match_type', 'Google Ads', customerId, { adGroupId, criterionId, newMatchType });
                const preview = approvalEnforcer.formatDryRunForDisplay(dryRun);
                return {
                    success: true,
                    requiresApproval: true,
            ...
            const result = await approvalEnforcer.validateAndExecute(confirmationToken, dryRun, async () => {
                ...
                await customer.adGroupCriteria.remove([removeOp]);
                ...
                const addResult = await customer.adGroupCriteria.create([addOp]);
                return {
            ...
            logger.error('Failed to update keyword match type', error);
            throw error;
        }
    },
};
