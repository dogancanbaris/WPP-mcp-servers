...
Would you like to proceed without dates or specify a date range?`;
      ...
      `;
            ...
            const totalCostMicros = adGroups.reduce((sum, ag) => sum + ag.costMicros, 0);
            ...
            const activeAdGroups = adGroups.filter((ag) => ag.adGroupStatus === 'ENABLED');
            ...
            const lowPerformers = activeAdGroups
                .filter((ag) => ag.impressions > 100 && ag.ctr < 0.5)
...
Full ad group data (${adGroups.length} ad groups) available in structured output.`;
            ...
            logger.error('Failed to get ad group performance', error);
            throw error;
        }
    },
};
//# sourceMappingURL=get-ad-group-performance.tool.js.map