...
import type { FormFactor, ProcessedCWV } from './types.js';
 ...
 */
function categorizeCWV(metric: string, p75: number): { category: string; emoji: string } {
  const thresholds: Record<
    string,
    { good: number; needsImprovement: number; unit?: string }
  > = {
 ...
 */
function generateCWVInsights(data: ProcessedCWV): string {
  ...
  if (data.lcp) {
    ...
    insights.push(
      `${lcpCat.emoji} **LCP (Largest Contentful Paint):** ${data.lcp.p75}ms - ${lcpCat.category}`
    );
    if (lcpCat.category === 'Poor') {
      insights.push(
        '   → Focus: Optimize largest image/text rendering, reduce server response time'
      );
    }
  }
  ...
  if (data.inp) {
    ...
    insights.push(
      `${inpCat.emoji} **INP (Interaction to Next Paint):** ${data.inp.p75}ms - ${inpCat.category}`
    );
  ...
  if (data.cls) {
    ...
    insights.push(
      `${clsCat.emoji} **CLS (Cumulative Layout Shift):** ${data.cls.p75.toFixed(3)} - ${clsCat.category}`
    );
    if (clsCat.category === 'Poor') {
      insights.push(
        '   → Focus: Set explicit sizes for images/ads, avoid dynamic content injection'
      );
    }
  }
  ...
  if (data.fcp) {
    ...
    insights.push(
      `${fcpCat.emoji} **FCP (First Contentful Paint):** ${data.fcp.p75}ms - ${fcpCat.category}`
    );
  }
  ...
  if (data.ttfb) {
    ...
    insights.push(
      `${ttfbCat.emoji} **TTFB (Time to First Byte):** ${data.ttfb.p75}ms - ${ttfbCat.category}`
    );
  }
 ...
 */
function compareFormFactors(results: Record<string, any>): string {
  const insights: string[] = [];

  const devices = ['DESKTOP', 'PHONE', 'TABLET'];
  const metricsToCompare: Array<{ key: keyof ProcessedCWV; name: string; unit: string }> = [
  ...
  for (const metric of metricsToCompare) {
    const values: Array<{ device: string; p75: number; category: string }> = [];
    ...
    for (const device of devices) {
      ...
      if (deviceData && !deviceData.error && deviceData[metric.key]) {
        ...
        const cat = categorizeCWV(metric.key as string, p75);
  ...
  async handler(input: any) {
    try {
      ...
      if (!input.url) {
        return injectGuidance(
          {},
          `📄 CORE WEB VITALS CHECK - SPECIFIC PAGE
  ...
  async handler(input: any) {
    try {
      ...
      if (!input.origin) {
        return injectGuidance(
          {},
          `📈 CORE WEB VITALS HISTORY - ORIGIN
  ...
  async handler(input: any) {
    try {
      ...
      if (!input.url) {
        return injectGuidance(
          {},
          `📈 CORE WEB VITALS HISTORY - SPECIFIC PAGE
...
What would you like to compare?`
      ...
      const availableDevices = Object.keys(results.comparison).filter(
        (device) => !results.comparison[device].error
      );
