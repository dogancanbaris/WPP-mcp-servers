...
5. Link to Google Ads`,
  ...
  async handler(input: any) {
    try {
      ...
      const propertyId = (result as any).name?.split('/')[1] || '';
...
3. Verify data is flowing (check real-time reports)`,
  ...
  async handler(input: any) {
    try {
      ...
      const streamId = (result as any).name?.split('/')[3] || '';
      const measurementId = (result as any).webStreamData?.measurementId || '';
  ...
  async handler(input: any) {
    try {
      ...
      dryRunBuilder.addRecommendation(
        `Ensure "${parameterName}" parameter is being sent in events`
      );
      dryRunBuilder.addRecommendation(
        'Data will only populate for events sent AFTER dimension creation'
      );
      ...
      return {
        ...
        data: {
          propertyId,
          dimensionId: (result as any).name,
          displayName,
          parameterName,
          scope,
