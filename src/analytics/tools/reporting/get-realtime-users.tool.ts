...
export const getRealtimeUsersTool = {
  ...
  async handler(input: any) {
    try {
      ...
      const dimensionNames = (response.dimensionHeaders || []).map((h: any) => h.name);
      const metricNames = (response.metricHeaders || []).map((h: any) => h.name);
      ...
      if (response.rows && response.rows.length > 0) {
        ...
        if (dimensionNames.length === 0) {
          ...
          totalActiveUsers = response.rows.reduce((sum: number, row: any) => {
      ...
      if (response.rows && response.rows.length > 0 && dimensionNames.length > 0) {
        const sorted = [...response.rows].sort((a: any, b: any) => {
