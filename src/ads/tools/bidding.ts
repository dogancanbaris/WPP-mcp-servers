...
Call this tool again with confirmationToken to execute.`;
      ...
      const result = await client.createBiddingStrategy(customerId, name, strategyType as any, targetMicros);
  ...
  async handler(input: any) {
    try {
      ...
      if (!strategyId) {
        ...
        const strategies = results.map((row: any) => {
          const strategy = row.bidding_strategy;
          return {
            id: String(strategy?.id || ''),
            name: String(strategy?.name || ''),
        ...
        if (strategies.length === 0) {
          return injectGuidance(
            { customerId },
            `⚠️ NO UPDATABLE STRATEGIES FOUND
...
What should the new target ROAS be (as decimal)?`;
        ...
        return injectGuidance(
          { customerId, strategyId, strategyType, currentTarget },
          guidanceText
        );
      }
  ...
  async handler(input: any) {
    try {
      ...
      if (!campaignId) {
        ...
        const manualCpcCampaigns = campaigns.filter((c: any) =>
          c.campaign?.bidding_strategy_type === 'MANUAL_CPC' ||
        ...
        if (manualCpcCampaigns.length === 0) {
          return injectGuidance(
            { customerId },
            `⚠️ NO MANUAL CPC CAMPAIGNS FOUND
...
What should the maximum CPC bid be (in dollars)?`;
        ...
        return injectGuidance(
          { customerId, campaignId, adGroupId, currentCpc, avgCpc },
          guidanceText
        );
      }
