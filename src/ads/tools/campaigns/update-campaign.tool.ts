  ...
  async handler(input: any) {
    try {
      const {
        ...
        startDate, endDate,
        trackingTemplate, finalUrlSuffix,
        confirmationToken
      } = input;
...
Provide the fields you want to update.`;
      ...
      const updateOperation = async () => {
        ...
        if (targetGoogleSearch !== undefined || targetSearchNetwork !== undefined ||
          ...
          if (targetGoogleSearch !== undefined) updates.network_settings.target_google_search = targetGoogleSearch;
          if (targetSearchNetwork !== undefined) updates.network_settings.target_search_network = targetSearchNetwork;
          if (targetContentNetwork !== undefined) updates.network_settings.target_content_network = targetContentNetwork;
          if (targetPartnerSearchNetwork !== undefined) updates.network_settings.target_partner_search_network = targetPartnerSearchNetwork;
          changeCount++;
        }
...
- Document reason for changes`;
      ...
      const changesList = [];
      if (name) changesList.push({ resource: 'Campaign', resourceId: campaignId, field: 'name', currentValue: 'current', newValue: name, changeType: 'UPDATE' });
      if (budgetId) changesList.push({ resource: 'Campaign', resourceId: campaignId, field: 'budget', currentValue: 'current', newValue: budgetId, changeType: 'UPDATE' });
      if (targetGoogleSearch !== undefined) changesList.push({ resource: 'Campaign', resourceId: campaignId, field: 'targetGoogleSearch', currentValue: 'current', newValue: String(targetGoogleSearch), changeType: 'UPDATE' });
      if (startDate) changesList.push({ resource: 'Campaign', resourceId: campaignId, field: 'startDate', currentValue: 'current', newValue: startDate, changeType: 'UPDATE' });
      if (endDate) changesList.push({ resource: 'Campaign', resourceId: campaignId, field: 'endDate', currentValue: 'current', newValue: endDate, changeType: 'UPDATE' });
      ...
      await audit.logFailedOperation('user', 'update_campaign', input.customerId, (error as Error).message, {
