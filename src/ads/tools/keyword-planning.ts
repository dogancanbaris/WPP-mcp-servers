  ...
  async handler(input: any) {
    try {
      ...
      const request: any = {
        ...
        geo_target_constants: geoTargetIds.map((id: string) => `geoTargetConstants/${id}`),
      };
      ...
      const ideas = (response as any).results || [];
      ...
      logger.error('Failed to generate keyword ideas', error as Error);
      throw error;
    }
  },
};
