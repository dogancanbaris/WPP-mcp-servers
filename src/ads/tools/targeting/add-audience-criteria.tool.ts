...
${EXAMPLE_IN_MARKET.slice(0, 4)
          .map((a: any) => `   • ${a.category}: ${a.description}`)
          .join('\n')}
...
**Provide:** audienceIds (array of audience resource names or IDs)`;
      ...
      logger.error('Failed to add audience criteria', error as Error);

      await audit.logFailedOperation('user', 'add_audience_criteria', input.customerId, (error as Error).message, {
