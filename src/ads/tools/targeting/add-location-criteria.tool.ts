...
**Common Countries:**
${COMMON_COUNTRIES.map((c: any) => `• ${c.name}: geoTargetIds: ["${c.id}"]`).join('\n')}

**Major US Cities (Examples):**
${US_MAJOR_CITIES.slice(0, 5).map((c: any) => `• ${c.name}: geoTargetIds: ["${c.id}"]`).join('\n')}
...
**Provide:** Either geoTargetIds (array of strings) OR radiusTargeting (object)`;
      ...
      logger.error('Failed to add location criteria', error as Error);
      ...
      await audit.logFailedOperation(
        ...
        { campaignId: input.campaignId, geoTargetIds: input.geoTargetIds }
      );
