...
**Provide:** Either preset (string) OR schedules (array of schedule objects)`;
      ...
      if (preset && SCHEDULE_PRESETS[preset as keyof typeof SCHEDULE_PRESETS]) {
        finalSchedules = SCHEDULE_PRESETS[preset as keyof typeof SCHEDULE_PRESETS].schedules;
      }
      ...
      if (!confirmationToken) {
        ...
        if (preset) {
          const presetInfo = SCHEDULE_PRESETS[preset as keyof typeof SCHEDULE_PRESETS];
        ...
        const groupedByDay = finalSchedules.reduce((acc: any, schedule: any) => {
        ...
        for (const day of DAYS_OF_WEEK) {
          ...
          if (daySchedules && daySchedules.length > 0) {
            const timeSlots = daySchedules
              .map(
                (s: any) =>
                  `${s.startHour}:${(s.startMinute || 0).toString().padStart(2, '0')}-${s.endHour}:${(s.endMinute || 0).toString().padStart(2, '0')}`
              )
              .join(', ');
        ...
        // Calculate coverage
        const totalHours = finalSchedules.reduce((sum: number, s: any) => {
      ...
      const operations: any[] = finalSchedules.map((schedule: any) => ({
      ...
      logger.error('Failed to set ad schedule', error as Error);

      await audit.logFailedOperation('user', 'set_ad_schedule', input.customerId, (error as Error).message, {
