...
\`\`\``;
      ...
      if (!matchType && !status && !maxCpcDollars) {
        ...
        const currentKeyword = keywords.find(
          (kw) => kw.ad_group_criterion?.resource_name === keywordResourceName
        );
...
What changes would you like to make?`;
      ...
      if (maxCpcDollars !== undefined) {
        ...
        if (Math.abs(percentChange) > 50) {
          dryRunBuilder.addRisk(
            `Large bid change (${percentChange > 0 ? '+' : ''}${percentChange.toFixed(0)}%) may cause significant traffic fluctuation`
          );
        }
      }
      ...
      if (!confirmationToken) {
        const { confirmationToken: token } = await approvalEnforcer.createDryRun(
          ...
          'Google Ads',
          customerId,
          { keywordResourceName, matchType, status, maxCpcDollars }
        );
        ...
        return {
          ...
          message:
            'Keyword update requires approval. Review the preview above and call this tool again with the confirmationToken to proceed.',
        };
      }
...
\`\`\``;
      ...
      dryRunBuilder.addRecommendation(
        'Keyword will stop serving immediately but can be re-enabled anytime without losing data'
      );
      dryRunBuilder.addRecommendation(
        'Consider alternatives: add negative keywords, lower bid, or change match type to EXACT'
      );
      ...
      if (!confirmationToken) {
        ...
        return {
          ...
          confirmationToken: token,
          message:
            'Keyword pause requires approval. Review the preview above and call this tool again with the confirmationToken to proceed.',
        };
      }
      ...
      logger.error('Failed to pause keyword', error as Error);
      throw error;
    }
  },
};
