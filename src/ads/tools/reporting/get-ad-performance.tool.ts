...
Would you like to proceed without dates or specify a date range?`;
      ...
      const totalCostMicros = ads.reduce((sum: number, ad: any) => sum + ad.costMicros, 0);
      const totalConversions = ads.reduce((sum: number, ad: any) => sum + ad.conversions, 0);
      const totalConversionValue = ads.reduce((sum: number, ad: any) => sum + ad.conversionValue, 0);
      ...
      const enabledAds = ads.filter((ad: any) => ad.adStatus === 'ENABLED');
      const topByImpressions = [...enabledAds]
        .sort((a, b) => b.impressions - a.impressions)
        .slice(0, 3);
      const topByConversions = [...enabledAds]
        .filter((ad: any) => ad.conversions > 0)
        .sort((a, b) => b.conversions - a.conversions)
        .slice(0, 3);
      const lowPerformers = enabledAds
        .filter((ad: any) => ad.impressions > 100 && ad.ctr < 0.5)
