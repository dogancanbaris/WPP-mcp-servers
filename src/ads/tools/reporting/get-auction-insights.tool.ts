...
export const getAuctionInsightsTool = {
  ...
  async handler(input: any) {
    try {
      ...
      const highOverlapCompetitors = insights.filter(i =>
        parseFloat(i.overlapRate) > 50
      ).length;
      const competitorsOutrankingYou = insights.filter(i =>
        parseFloat(i.positionAboveRate) > 50
      ).length;
...
${formatNextSteps([
  ...
  'Adjust budgets: If losing to budget, increase daily budget; if losing to rank, improve QS or raise bids'
      ...
      logger.error('Failed to get auction insights', error as Error);
      throw error;
    }
  },
};
