...
What percentage adjustment for ${locationName}?`;
      ...
      if (Math.abs(bidModifierPercent) >= 50) {
        dryRunBuilder.addRecommendation(
          `⚠️ Large bid adjustment (${bidModifierPercent}%) for ${locationName}. This will significantly impact spend in this location.`
        );
      }

      if (bidModifierPercent === -100) {
        dryRunBuilder.addRecommendation(
          `🚫 EXCLUSION: Ads will NOT show in ${locationName}. Ensure this is intentional.`
        );
      }

      if (Math.abs(bidModifierPercent) < 10) {
        dryRunBuilder.addRecommendation(
          `ℹ️ Small adjustment (${bidModifierPercent}%). May have minimal impact on ${locationName} performance.`
        );
      }

      dryRunBuilder.addRecommendation(
        `📊 Monitor ${locationName} performance after this change. Check "Locations" report in 14-30 days.`
      );

      dryRunBuilder.addRecommendation(
        `🌍 Consider other nearby locations. If ${locationName} performs well, adjacent areas might too.`
      );
      ...
      if (!confirmationToken) {
        const { confirmationToken: token } = await approvalEnforcer.createDryRun(
          ...
          'Google Ads',
          customerId,
          { campaignId, locationId, locationName, bidModifierPercent }
        );
        ...
        return {
          ...
          message:
            'Location bid modifier creation requires approval. Review the preview and call this tool again with confirmationToken.',
        };
      }
      ...
      logger.error('Failed to create location bid modifier', error as Error);
