...
export const listAdsTool = {
  ...
  async handler(input: any) {
    try {
      ...
      // Analyze ads
      const statusCounts = aggregatedAds.reduce((acc: any, ad: any) => {
        const status = ad.ad?.status || 'UNKNOWN';
        acc[status] = (acc[status] || 0) + 1;
        return acc;
      }, {});

      const approvalCounts = aggregatedAds.reduce((acc: any, ad: any) => {
      ...
      // Calculate totals
      const totalImpressions = aggregatedAds.reduce((sum: number, ad: any) => sum + ad.metrics.impressions, 0);
      const totalClicks = aggregatedAds.reduce((sum: number, ad: any) => sum + ad.metrics.clicks, 0);
      const totalCost = aggregatedAds.reduce((sum: number, ad: any) => sum + ad.metrics.cost_micros, 0);
      const avgCtr = totalImpressions > 0 ? (totalClicks / totalImpressions) * 100 : 0;

      // Format ads for display
      const formatAd = (adData: any, index: number) => {
...
${formatNextSteps([
  ...
  'Review keywords: use get_keyword_performance'
      ...
      logger.error('Failed to list ads', error as Error);
      throw error;
    }
  },
};
