...
\`\`\``;
      ...
      logger.error(`Failed to ${input.action || 'pause/enable'} ad`, error as Error);

      // AUDIT: Log failed operation
      await audit.logFailedOperation('user', 'pause_ad', input.customerId, (error as Error).message, {
