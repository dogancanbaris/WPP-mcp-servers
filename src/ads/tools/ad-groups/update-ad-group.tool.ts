...
What would you like to change?`;
        ...
        return injectGuidance(
          { customerId, campaignId, adGroupId, currentAdGroup },
          guidanceText
        );
      }
      ...
      if (status && status !== currentAdGroup?.status) {
        ...
        if (status === 'REMOVED') {
          dryRunBuilder.addRisk(
            '🚨 PERMANENT DELETION: This ad group will be permanently removed and cannot be restored'
          );
        } else if (status === 'ENABLED' && currentAdGroup?.status === 'PAUSED') {
          dryRunBuilder.addRecommendation(
            '⚠️ Enabling ad group will resume ad serving immediately. Ensure ads and keywords are ready.'
          );
        } else if (status === 'PAUSED' && currentAdGroup?.status === 'ENABLED') {
          dryRunBuilder.addRecommendation(
            '⚠️ Pausing ad group will stop all ads in this group immediately'
          );
        }
      }
      ...
      if (cpcBidMicros !== undefined && cpcBidMicros !== currentAdGroup?.cpc_bid_micros) {
        ...
        if (Math.abs(parseFloat(percentChange)) > 50) {
          dryRunBuilder.addRisk(
            `Large CPC bid change (${percentChange}%) may cause delivery fluctuations`
          );
          dryRunBuilder.addRecommendation(
            'Consider making CPC bid changes in smaller increments (10-20% at a time)'
          );
        }
      }
      ...
      if (urlCustomParameters && urlCustomParameters.length > 0) {
        dryRunBuilder.addChange({
          ...
          currentValue: 'Current',
          newValue: urlCustomParameters.map((p: any) => `${p.key}=${p.value}`).join(', '),
          changeType: 'update',
        });
      }
      ...
      if (status !== 'REMOVED') {
        dryRunBuilder.addRecommendation(
          'Monitor performance for 24-48 hours after making changes'
        );
      }
      ...
      if (!confirmationToken) {
        const { confirmationToken: token } = await approvalEnforcer.createDryRun(
          ...
          'Google Ads',
          customerId,
          { adGroupId, name, status, cpcBidMicros, type, trackingUrlTemplate, urlCustomParameters, adRotationMode }
        );
        ...
        return {
          ...
          message:
            'Ad group update requires approval. Review the preview above and call this tool again with the confirmationToken to proceed.',
        };
      }
      ...
      const result = await approvalEnforcer.validateAndExecute(
        ...
        async () => {
          ...
          if (urlCustomParameters && urlCustomParameters.length > 0) {
            updateOperation.url_custom_parameters = urlCustomParameters.map((param: any) => ({
              key: param.key,
              value: param.value,
            }));
          }
      ...
      logger.error('Failed to update ad group', error as Error);

      // AUDIT: Log failed ad group update
      await audit.logFailedOperation('user', 'update_ad_group', input.customerId, (error as Error).message, {
