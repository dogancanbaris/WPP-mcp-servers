...
\`\`\``;
   ...
   Type: ${campaign?.advertising_channel_type}`;
      ...
      if (!name) {
        const campaigns = await client.listCampaigns(customerId);
        const selectedCampaign = campaigns.find((c: any) => c.campaign.id === campaignId);
...
Would you like to set a CPC bid? (Optional - leave empty to skip)`;
        ...
        return injectGuidance(
          { customerId, campaignId, name, skipCpcBid: true },
          guidanceText
        );
      }
      ...
      if (status === 'ENABLED') {
        dryRunBuilder.addRecommendation(
          '⚠️ Ad group will be created in ENABLED status. Ensure you have keywords and ads ready to add.'
        );
      } else {
        dryRunBuilder.addRecommendation(
          '✅ Ad group will be created in PAUSED status. Add keywords and ads before enabling.'
        );
      }

      dryRunBuilder.addRecommendation(
        'Next steps: Add 5-20 keywords to this ad group, then create 2-3 ads for A/B testing'
      );
      ...
      if (!confirmationToken) {
        const { confirmationToken: token } = await approvalEnforcer.createDryRun(
          ...
          'Google Ads',
          customerId,
          { campaignId, name, cpcBidMicros, status, type, trackingUrlTemplate, urlCustomParameters, adRotationMode }
        );
        ...
        return {
          ...
          message:
            'Ad group creation requires approval. Review the preview above and call this tool again with the confirmationToken to proceed.',
        };
      }
      ...
      const result = await approvalEnforcer.validateAndExecute(
        ...
        async () => {
          ...
          if (urlCustomParameters && urlCustomParameters.length > 0) {
            adGroupOperation.url_custom_parameters = urlCustomParameters.map((param: any) => ({
              key: param.key,
              value: param.value,
            }));
          }
      ...
      logger.error('Failed to create ad group', error as Error);

      // AUDIT: Log failed ad group creation
      await audit.logFailedOperation('user', 'create_ad_group', input.customerId, (error as Error).message, {
