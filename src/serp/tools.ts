...
${organicResults.slice(0, 5).map((result: any, i: number) =>
  `${i + 1}. **${result.title}**
   URL: ${result.link}
...
${peopleAlsoAsk.slice(0, 3).map((q: any) => `   • ${q.question}`).join('\n')}
...
${relatedSearches.slice(0, 5).map((s: any) => `   • ${s.query}`).join('\n')}
...
**Top 3 Domains:**
${organicResults.slice(0, 3).map((r: any, i: number) =>
  `${i + 1}. ${r.domain || new URL(r.link).hostname}`
).join('\n')}

**Your Position:**
${organicResults.findIndex((r: any) => r.link.includes('your-domain.com')) + 1 || 'Not in top results'}
