 ...
 */
const DEFAULT_CONFIG: GSCConfig = {
...
class ConfigManager {
  ...
  async load(): Promise<void> {
    try {
      const fileConfig = await readJsonFile<GSCConfig>(this.configPath);
      ...
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
        ...
        throw new ConfigurationError(
          `Failed to load configuration from ${this.configPath}: ${(error as Error).message}`
        );
      }
    }
  }
    ...
    if (this.config.selectedProperties) {
      for (const property of this.config.selectedProperties) {
        try {
          ...
          throw new ConfigurationError(`Invalid property in configuration: ${(error as Error).message}`);
        }
      }
    }
  ...
  async save(): Promise<void> {
    try {
      ...
      throw new ConfigurationError(
        `Failed to save configuration to ${this.configPath}: ${(error as Error).message}`
      );
    }
  }
   ...
   */
  async addProperty(property: string): Promise<void> {
    try {
      validateGSCProperty(property);
    } catch (error) {
      throw new ConfigurationError(`Invalid property: ${(error as Error).message}`);
    }
   ...
   */
  async removeProperty(property: string): Promise<void> {
    if (!this.config.selectedProperties) {
      return;
    }
   ...
   * Set role
   */
  async setRole(role: 'admin' | 'editor' | 'viewer'): Promise<void> {
