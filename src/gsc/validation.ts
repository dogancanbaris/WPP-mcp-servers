...
export const GSCPropertySchema = z
  ...
  .regex(
    /^sc-(domain|https?|app):/,
    'Invalid GSC property format. Expected format like "sc-domain:example.com" or "sc-https://example.com"'
  );
...
export function validateGSCProperty(property: string): void {
  try {
    ...
    throw new Error(`Invalid GSC property format: ${(error as Error).message}`);
  }
}
