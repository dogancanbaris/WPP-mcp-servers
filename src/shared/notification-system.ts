...
export class NotificationSystem {
  ...
  private pendingAgencyNotifications: Notification[] = [];
  ...
  async initialize(config: NotificationConfig): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to initialize notification system', error as Error);
      throw error;
    }
   ...
   * Send notification
   */
  async notify(notification: Omit<Notification, 'id' | 'timestamp' | 'sentToCentral' | 'sentToAgency'>): Promise<void> {
   ...
   */
  private async sendToCentralAdmin(notification: Notification): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to send notification to central admin', error as Error);
    }
  }
   ...
   */
  private queueForAgencyManagers(notification: Notification): void {
   ...
   */
  private async processAgencyBatch(): Promise<void> {
    ...
    const notificationsByManager = new Map<string, Notification[]>();

    for (const notification of this.pendingAgencyNotifications) {
      // Find agency managers responsible for this account
      const managers = this.config.agencyManagers.filter((manager) =>
        manager.accountIds.includes(notification.accountId)
      );
   ...
   */
  private async sendBatchToAgencyManager(email: string, notifications: Notification[]): Promise<void> {
   ...
   * Build email subject
   */
  private buildSubject(notification: Notification): string {
   ...
   */
  private buildBatchEmailBody(notifications: Notification[]): string {
   ...
   */
  private buildBatchHtmlBody(notifications: Notification[]): string {
