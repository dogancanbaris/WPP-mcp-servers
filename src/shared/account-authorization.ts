...
export class AccountAuthorizationManager {
  private approvedAccounts: ApprovedAccount[] = [];
  ...
  canAccessAccount(api: string, accountId: string): boolean {
    return this.approvedAccounts.some(
      (acc) => acc.api === api && acc.accountId === accountId
    );
  }
  ...
  enforceAccess(api: string, accountId: string): void {
    if (!this.canAccessAccount(api, accountId)) {
      ...
      throw new UnauthorizedAccountAccessError(
        `You don't have access to ${api} account ${accountId}.\n\n` +
  ...
  getApprovedAccountsList(): string {
    ...
    return this.approvedAccounts
      .map(
        (acc) =>
          `  - ${acc.api}: ${acc.accountId}${acc.accountName ? ` (${acc.accountName})` : ''}${acc.clientName ? ` - ${acc.clientName}` : ''}`
      )
      .join('\n');
  }
   ...
   */
  private decrypt(encrypted: string, secret: string): string {
    try {
...
let accountAuthManagerInstance: AccountAuthorizationManager | null = null;

/**
 * Get account authorization manager instance
 */
export function getAccountAuthorizationManager(): AccountAuthorizationManager {
 ...
 */
export function setAccountAuthorizationManager(manager: AccountAuthorizationManager): void {
