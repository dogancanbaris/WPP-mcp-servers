...
export class ChangeHistoryManager {
  ...
  async queryChangeHistory(params: {
    ...
    try {
      ...
      logger.error('Failed to query change history', error as Error);
      throw new Error(`Failed to query change history: ${(error as Error).message}`);
    }
  }
  ...
  async verifyOperation(params: {
    ...
    resourceType: string;
    resourceName: string;
  }): Promise<{ verified: boolean; changeEvent?: ChangeEvent; message: string }> {
    ...
    const matchingEvent = history.events.find((event) =>
      event.changeResourceName.includes(params.resourceName)
    );
  ...
  async getChangesForRollback(params: {
    ...
    const resourceChanges = history.events.filter((event) =>
      event.changeResourceName.includes(params.resourceName)
    );
...
// Singleton instance
let changeHistoryManagerInstance: ChangeHistoryManager | null = null;

/**
 * Get change history manager instance
 */
export function getChangeHistoryManager(): ChangeHistoryManager {
...
export async function queryAndFormatChangeHistory(params: {
  ...
  resourceType?: string;
  campaignId?: string;
}): Promise<{ result: ChangeHistoryResult; report: string }> {
