...
export class PatternMatcher {
  private readonly MAX_BULK_ITEMS = 20; // User requirement: max 20 items
  ...
  }): PatternMatchResult<{ id: string; name: string; status: string }> {
    return this.match({
      pattern: params.pattern,
  ...
  matchKeywords(params: {
    pattern: string;
    keywords: Array<{ text: string; matchType: string; status: string }>;
  }): PatternMatchResult<{ text: string; matchType: string; status: string }> {
    return this.match({
      pattern: params.pattern,
  ...
  createBulkOperation<T>(params: {
    ...
    matchedItems: T[];
    formatFn: (item: T) => string;
  }): { requiresConfirmation: true; confirmationMessage: string; itemsToConfirm: T[] } {
    ...
    if (totalItems > this.MAX_BULK_ITEMS) {
      throw new TooManyMatchesError(
        `Cannot perform bulk operation on ${totalItems} items (max: ${this.MAX_BULK_ITEMS}). Please refine your pattern.`
      );
    }
