  ...
  async handler(input: any) {
    try {
      ...
      if (!accountId) {
        ...
        const accountsResponse = await (client as any).accounts.list();
        const accounts = accountsResponse.data.accounts || [];

        if (accounts.length === 0) {
          return injectGuidance(
            { accounts: [] },
            `⚠️ NO BUSINESS PROFILE ACCOUNTS FOUND
...
${formatted.length > 0
  ? formatted.map((loc, i) => `${i + 1}. ${loc.title || 'Unnamed Location'}
   ...
   Categories: ${loc.categories?.map((c: any) => c.displayName).join(', ') || 'None'}`)
    .join('\n\n')
  : '(No locations found)'}
...
**CATEGORIES:**
${(Array.isArray(location.categories) ? location.categories : []).map((c: any) => `- ${c.displayName}`).join('\n') || 'None'}

**BUSINESS HOURS:**
${(location.regularHours?.periods?.length ?? 0) > 0
  ? location.regularHours?.periods?.map((p: any) =>
    `${p.openDay}: ${p.openTime?.hours || '00'}:${p.openTime?.minutes || '00'} - ${p.closeTime?.hours || '00'}:${p.closeTime?.minutes || '00'}`)
    .join('\n')
  : 'Not set'}
...
**Business Hours:**
${loc.regularHours?.periods?.map((p: any) =>
  `   • ${p.openDay}: ${p.openTime?.hours || '00'}:${p.openTime?.minutes || '00'} - ${p.closeTime?.hours || '00'}:${p.closeTime?.minutes || '00'}`
).join('\n') || '   Not set'}
...
\`\`\``;
        ...
        return injectGuidance(
          { currentLocation: loc, nextParam: 'updates' },
          guidanceText
        );
      }
      ...
      Object.keys(updates).forEach((field) => {
        dryRunBuilder.addChange({
          ...
          field,
          currentValue: JSON.stringify(current[field as keyof typeof current] || 'N/A'),
